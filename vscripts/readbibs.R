...
]$author <- "Suzanne Hoogeveen and Alexandra Sarafoglou and Balazs Aczel ... and Vince Polito et al."
...
create_md <- function(x) {
    # start writing
    ifelse(
        !dir.exists(file.path(x[["extrapath"]])),
        dir.create(file.path(x[["extrapath"]])),
        FALSE
    )
    ...
    if (
        ...
        write(
            paste0(
                "date = \"",
                dmy(paste(01, x[["month"]], x[["year"]], sep = "-")),
                "\""
            ),
            fileConn,
            append = T
